...
| 🔍 **快速搜索** | 内置站内模糊搜索，支持关键词高亮、`/` 快捷键聚焦与无结果提示；可自定义搜索引擎（拖拽排序、新增任意引擎） |
| 📂 **分类清晰** | 通过多级分类组织书签，浏览直观高效；支持按默认分类自动隐藏「全部」Tab |
| 🔒 **安全后台** | 基于 KV 的管理员认证，提供完整的书签增删改查后台 |
| 📝 **用户提交** | 支持访客提交书签，经管理员审核后显示（可通过环境变量关闭） |
| ⚡ **性能卓越** | 利用 Cloudflare 边缘缓存，秒级加载，节省 D1 数据库读取成本；图标走 CDN 边缘缓存，不占用 KV 配额 |
| 🖼️ **卡片体验** | 悬停卡片显示完整描述（原生 tooltip）；图标自动提取（GitHub 项目自动用仓库 Owner 头像） |
| ⬆️ **本地上传** | 支持上传自定义壁纸与站点 Favicon（存储于 R2，不占用 KV） |
| 📤 **数据管理** | 支持书签数据的导入与导出，兼容 Chrome 导出的 HTML 格式，导入时自动补全图标 |
...
- 🖼️ **Logo 自动生成**：默认使用 [faviconsnap.com](https://faviconsnap.com) 接口，可在环境变量中自定义；**GitHub 项目自动使用仓库 Owner 头像作为图标**（无需手填 Logo）。
- 📦 **导入导出数据**：提供书签数据的导入与导出，支持 Chrome 导出的 HTML 格式一键导入（导入时自动补全图标），也可直接从[公共书签库](https://github.com/jy02739244/bookmark-library)选取现成书签集导入。
- 🔎 **搜索体验升级**：站内搜索支持关键词高亮、`/` 快捷键聚焦、无结果提示；「站外搜索」引擎支持拖拽排序与自定义新增（URL 模板含 `{q}` 占位符）。
- 🖼️ **卡片悬停描述**：鼠标悬停书签卡片时，通过原生 tooltip 展示完整描述（有描述显示描述、无描述显示名称）。
- ⬆️ **壁纸 / Favicon 本地上传**：后台可直接上传壁纸与站点 Favicon，存储于 Cloudflare R2（10GB 免费额度），不再占用 KV 配额；支持上传图片删除与一键应用。
- ⚡ **图标缓存优化**：书签图标改走 Cloudflare CDN 边缘缓存 + 浏览器长缓存，不再消耗 KV 读写配额，大幅降低账户 KV 用量。
...
### 步骤 5：创建 R2 存储桶（壁纸 / Favicon 上传存储，推荐）

> 自 2026-08 起，壁纸与本地上传的 Favicon 存储于 R2 对象存储（不再占用 KV）。**若不使用壁纸/Favicon 上传功能，可跳过此步骤**；绑定后即可完整使用相关功能。

1. 在 Cloudflare 控制台，进入 `存储和数据库` → `R2 对象存储`。
2. 点击 `创建存储桶`，存储桶名称输入 `nav-images`（可自定义，需与后续绑定一致），然后创建。
...
4. 点击 `添加绑定`，选择 `R2 存储桶`（已创建 R2 时）：
    - 变量名称：`NAV_IMG`
    - 存储桶：选择你创建的 `nav-images`
5. 如需使用 Cloudflare Workers AI，继续点击 `添加绑定`，选择 `Workers AI`：
    - 变量名称：`AI`
...
| 绑定名 | 类型 | 说明 |
| :--- | :--- | :--- |
| `NAV_IMG` | R2 存储桶 | 壁纸与本地上传 Favicon 的存储（如使用上传功能则必需） |
...
<!-- changelog:start -->
- 🚀 **2026-08-21**：搜索体验升级（关键词高亮 / `/` 快捷键 / 无结果提示 / 自定义搜索引擎）；卡片悬停显示完整描述；壁纸与 Favicon 本地上传（R2 存储）；图标改用边缘缓存不再占用 KV；GitHub 项目自动使用 Owner 头像；导入自动补全图标；「全部」Tab 按默认分类自动隐藏；页脚 GitHub 链接与站点 Favicon 可配置
